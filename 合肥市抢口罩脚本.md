...
function request(type, path, param, callback) {
  var options = {
    encoding: null,
    hostname: "kzgm.bbshjz.cn",
    port: 8000, //端口号 https默认端口 443， http默认的端口号是80
    path: path,
    method: type,
    headers: {
  ...
  req.write(JSON.stringify(param)); //post 请求传参
  req.end(); //必须要要写，
}

var urlFirst = "http://kzgm.bbshjz.cn:8000/ncms";
// code 11183 是坝下路国胜大药房的编号
var code = "11183";
var startDate = null
...
function getMask(arr) {
  ...
  if (maskObj) {
    var data = {
      ...
      reservationNumber: "5",
      // 你要的药店名称
      pharmacyName: "国胜大药房坝下路店",
      // 药店id
      pharmacyCode: code,
      // 通过上次请求获取到的有效的口罩信息 数量和时间点
      pharmacyPhase: maskObj.value,
      pharmacyPhaseName: maskObj.text
    };
    var url = isvData(urlFirst + "/mask/book");
    request("POST", url, data, function(res) {
      console.log( res, "请求完成,用时" + (new Date().getTime()- Date.parse(startDate) ) + 'ms')
    });
  }
}
...
// 第一步 获取当前口罩剩余数量
startDate = new Date()
console.log("开始请求" + startDate.toLocaleString());
request(
  "GET",
  isvData(urlFirst + "/mask/pharmacy-stock?code=" + code),
  null,
  function(data) {
