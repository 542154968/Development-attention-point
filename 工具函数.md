...
// 按钮
        toggleSize() {
            this.$refs.map && this.fullScreen(this.$refs.map);
        },
        fullScreen(el) {
            //进入全屏  退出全屏
        ...
        resetMap(toFull = false) {
            let $map = document.getElementById("BMap-container-ht");
            if (toFull) {
                $map.style.height = window.screen.height + 'px';
                $map.style.width = window.screen.width + 'px';
                this.isFull = true;
            } else {
                $map.style.height = '740px';
        ...
        toggleChange() {
            ...
            this.reseizeTimeId = setTimeout(() => {
                if (window.navigator.userAgent.indexOf(".NET ") > -1) {
                    ...
                    !this.isFullScreen() && this.resetMap();
                }
                this.mapToCenter();
            }, 200)
