...
var xa = {
	...
	dataTables:function(elem,ajaxUrl,columnArr,drawCallback,option){
		...
		option.searchCols = option.searchCols || [];
        var language = {
            sProcessing: "数据加载中...",
            sLengthMenu: "显示 _MENU_ 项结果",
            sZeroRecords: "暂无匹配结果",
...
$.fn.updateViewer = function (){
	...
	$(document.body).append('<div id="viewer-js" style="display:none"></div>');
...
/*
	单张图片查看器 viewer.js
 */
function showViewerImg(event){	
	var event= window.event || arguments.callee.caller.arguments[0];
	var target = event.target || window.event.target;
	$(target).parents('.viewer-img-container').updateViewer();
	var viewer = new Viewer(document.getElementById('viewer-js'), {
	    url: 'data-original',
	    fullscreen:false,
	    navbar:false,
	...
	$('#viewer-js img').eq(0).trigger('click');
}
...
function hideIframeModal(){
	try{
		if($('#iframeModal').hasClass('in')){
			$('#iframeModal').modal('hide');
		}
		dataTable.draw(false);
		$('body').css('overflow', 'visible')
	    $('.modal-open .modal').css('overflow-y','visible')
	}catch(error){}
}
