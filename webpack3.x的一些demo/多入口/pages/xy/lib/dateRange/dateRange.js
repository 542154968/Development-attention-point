...
function pickerDateRange(inputId, options) {
    ...
    var defaults = {
        ...
        aRecentThisWeek: 'aRecentThisWeek', //本周
        aRecentLastWeek: 'aRecentLastWeek',  //上周
        aRecentThisMonth: 'aRecentThisMonth',  //本月
        aRecentLastMonth: 'aRecentLastMonth',  //上月

        isShowToday:true,// 是否默认当天
        startDate: '', // 开始日期
        endDate: '', // 结束日期
    ...
    this.periodObj[__method.mOpts.aRecentThisWeek] = 'bz';
    this.periodObj[__method.mOpts.aRecentLastWeek] = 'sz';
    this.periodObj[__method.mOpts.aRecentThisMonth] = 'by';
    this.periodObj[__method.mOpts.aRecentLastMonth] = 'sy';
    ...
    var wrapper = {
        ...
        ta: [
            ...
            '</div>',
            '<div class="frm_btn">',
            '<input class="ta_btn" type="button" style="margin-right: 150px" id="' + this.clearBtn + '" value="清空日期" />',
    ...
    var checkBoxWrapper = {
        gri: [
            '<label class="checkbox pull-left" style="padding: 9px 10px 0 10px">'+
                '<input type="checkbox" id="'+this.compareCheckboxId+'">'+
                '<span class="text-middle" style="top:0"><i class="icon-check"></i></span><span style="font-size:13px;position:relative;left:5px;top:-2px">对比时间段</span>'+
            '</label>'+
            '<input type="text" name="'+this.inputCompareId+'" id="'+this.inputCompareId+'" value="" class="form-control form-date-more">'
    ...
    // }
    $(checkBoxWrapper['gri'].join('')).insertAfter($('#' + this.inputId));
    //根据传入参数决定是否展示日期输入框
    ...
    for (var property in __method.periodObj) {
        ...
        if ($('#' + property).length > 0) {
            ...
            $('#' + property).bind('click', function() {
                //处理点击样式
                var cla = 'active';
                $(this).parent().find('.'+cla).removeClass(cla);
                $(this).addClass(cla);
                //拼接提交时间串

                var timeObj = __method.getSpecialPeriod(__method.periodObj[$(this).attr('id')]);
                // console.log(__method.getSpecialPeriod)
                ...
                $('#' + __method.compareCheckboxId).prop('checked', false);
                $('#' + __method.mOpts.compareCheckboxId).prop('checked', false);
                $('#' + __method.compareCheckboxId).trigger('click');
                $('#' + __method.mOpts.compareCheckboxId).trigger('click');
                $('#' + __method.compareCheckboxId).prop('checked', false);
                $('#' + this.compareInputDiv).css('display', $('#' + this.compareCheckboxId).attr('checked') ? '' : 'none');
                __method.close(1);
...
pickerDateRange.prototype.bindChangeForSelect = function() {
    ...
    * @param month 月
    **/
    pickerDateRange.prototype.getPriorMonthFirstDay = function (year, month) {
    ...
    * @param month月份
    * */
    pickerDateRange.prototype.getMonthDays = function (year, month) {
        //本月第一天 1-31  
...
pickerDateRange.prototype.getSpecialPeriod = function(period) {
    ...
    switch(period){
        ...
        case 'bz':
                ...
                //添加本周时间  
                todayStr = formatDate(monday);   
                //添加本周最后一天时间  
                otherdayStr = formatDate(sunday); 
                break;
        case 'sz':
                //上周起止日期  
                //获取当前时间  
                ...
                todayStr = formatDate(priorWeekFirstDay); 
                //添加本周最后一天时间  
                otherdayStr = formatDate(priorWeekLastDay);
                break;
        case 'by':
                //本月起止日期  
                //获取当前时间  
                ...
                todayStr = formatDate(firstDay); 
                //添加本周最后一天时间  
                otherdayStr = formatDate(lastDay);
                break;
        case 'sy':
                //上月起止日期  
                //获取当前时间  
                ...
                var currentYear = currentDate.getFullYear();
                //获得上一个月的第一天  
                var priorMonthFirstDay = __method.getPriorMonthFirstDay(currentYear, currentMonth);
                //获得上一月的最后一天  
                var priorMonthLastDay = new Date(priorMonthFirstDay.getFullYear(), priorMonthFirstDay.getMonth(), __method.getMonthDays(priorMonthFirstDay.getFullYear(), priorMonthFirstDay.getMonth()));
                todayStr = formatDate(priorMonthFirstDay);
                //添加本周最后一天时间  
                otherdayStr = formatDate(priorMonthLastDay);
                break;
        default:
